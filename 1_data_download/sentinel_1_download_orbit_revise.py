...
locations = ['4553_3325_13','4622_3159_13','572_3643_13','6688_3456_13','6813_3313_13','7517_4908_13']
orbit_info = []
for item in locations:
    zip_files = glob.glob('../data/Sentinel-1/'+item+'/original_dat/*/*.zip')
    tmp = []
    for zip_file in zip_files:
        info = zip_file.split('/')[-1].split('_')
        if info[0] == 'S1A':
            tmp.append(int(np.mod(np.double(info[-3])-73,175)+1))
        elif info[0] == 'S1B':
            tmp.append(int(np.mod(np.double(info[-3])-27,175)+1))
    orbit_info.append(tmp)
...
# loop over cities
for idx_city in range(len(cityname)-1,-1,-1):
    ...
    ###############################################################################    
    for tt in range(len(startDate)-1,-1,-1):
        ###############################################################################
        ...
        for index, row in products_df.iterrows():
            relative_orbit = row['relativeorbitnumber']
            if relative_orbit not in orbit_info[locations.index(city)]:
                continue
