...
def createGPTTemplate(inputZip, template, geoRegion, region, procFlag, projFlag, projection=0):
    ...
    ok_file = outputPath + "/ok.process"
    if os.path.exists(ok_file):
        print(outputData + " has been processed.")
        return 3, outputData
...
def getGeoRegionGeojson(cpath, geojson_file):
    ...
    #       -- kmlCityList  - path to the ROI geojson file 
    #
    # Output
    ...
    roi_id = cpath.split('/')[-1]
    # get ROIs
    f = ogr.Open(geojson_file)
    lye = f.GetLayer()
    nb_rois = lye.GetFeatureCount()
    for i in range(nb_rois):
        feat = lye.GetFeature(i)
        if feat.GetField(0) == roi_id:
            geom = feat.geometry()
            roi_ext = geom.ExportToWkt()
            break

    x = np.zeros([5,1])
    y = np.zeros([5,1])
    tmp = roi_ext.split('((')[-1].split('))')[0].split(',')
    ...
    return geoRegion, centerPoint, region
